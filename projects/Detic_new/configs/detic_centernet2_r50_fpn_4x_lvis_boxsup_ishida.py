...
# custom_imports = dict(imports=["projects.Detic_new.detic"], allow_failed_imports=False)

# # ------------------------------------------------------
# # num_classes = 1203
# num_classes = 3
# CLASS_WEIGHTS = [1.0, 1.0, 1.0, 0.1]
# DATA_ROOT = "C:/Users/s.yamagami/Documents/works/classification_tunas/datasets/Phase0dash_80images"
# DATA_METADATA_DIR = "C:/Users/s.yamagami/Documents/works/classification_tunas/data/metadata"
...
# lvis_cat_frequency_info = f"{DATA_METADATA_DIR}/ishida_train_cat_info.json"
# # USE_FED_LOSS = False
# USE_FED_LOSS = True
...
#     zs_weight_path=f"{DATA_METADATA_DIR}/ishida_clip_a+cname.npy",
#     zs_weight_dim=512,
#     use_bias=0.0,
#     norm_weight=True,
...
#                 loss_cls=dict(type="CrossEntropyLoss", use_sigmoid=True, loss_weight=1.0, class_weight=CLASS_WEIGHTS),
...
#                 cat_freq_path=lvis_cat_frequency_info,
#                 bbox_coder=dict(
#                     type="DeltaXYWHBBoxCoder", target_means=[0.0, 0.0, 0.0, 0.0], target_stds=[0.033, 0.033, 0.067, 0.067]
#                 ),
#                 reg_class_agnostic=True,
#                 loss_cls=dict(type="CrossEntropyLoss", use_sigmoid=True, loss_weight=1.0, class_weight=CLASS_WEIGHTS),
#                 loss_bbox=dict(type="SmoothL1Loss", beta=0.1, loss_weight=1.0),
...
#             class_agnostic=True,
#             num_classes=num_classes,
#             loss_mask=dict(type="CrossEntropyLoss", use_mask=True, loss_weight=1.0, class_weight=CLASS_WEIGHTS),
#         ),
#     ),
...
#         ),
#         rpn_proposal=dict(
#             score_thr=0.0001, nms_pre=4000, max_per_img=2000, nms=dict(type="nms", iou_threshold=0.9), min_bbox_size=0
#         ),
#         rcnn=[
#             dict(
#                 assigner=dict(
...
#     ),
#     test_cfg=dict(
#         rpn=dict(score_thr=0.0001, nms_pre=1000, max_per_img=256, nms=dict(type="nms", iou_threshold=0.9), min_bbox_size=0),
#         rcnn=dict(score_thr=0.02, nms=dict(type="nms", iou_threshold=0.5), max_per_img=300, mask_thr_binary=0.5),
#     ),
# )
...
#     dict(type="RandomCrop", crop_type="absolute_range", crop_size=(640, 640), recompute_bbox=True, allow_negative_crop=True),
...
# test_pipeline = [
#     dict(type="LoadImageFromFile", backend_args=backend_args, imdecode_backend=backend_args),
#     dict(type="Resize", scale=(1333, 800), keep_ratio=True, backend=backend_args),
...
# # ------------------------------------------------------
# # val_evaluator = dict(type="LVISMetric", ann_file="data/lvis/annotations/lvis_v1_val.json", metric=["bbox", "segm"])
# val_evaluator = dict(
#     type="CocoMetric",
#     ann_file=f"{DATA_ROOT}/{ANN_FILE_VAL}",
#     metric=["bbox", "segm"],
# )
# # ------------------------------------------------------
...
# max_iter = MAX_ITER
# train_cfg = dict(type="IterBasedTrainLoop", max_iters=max_iter, val_interval=90000)
...
# param_scheduler = [
#     dict(type="LinearLR", start_factor=0.0001, by_epoch=False, begin=0, end=10000),
#     dict(
...
# default_hooks = dict(
#     # ------------------------------------------------------
#     # checkpoint=dict(by_epoch=False, interval=30000, max_keep_ckpts=5), logger=dict(type="LoggerHook", interval=50)
#     checkpoint=dict(by_epoch=False, interval=CHECKPOINT_INTERVAL, max_keep_ckpts=5),
